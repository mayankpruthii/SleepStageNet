...
def deepSleepNetwork(pre_model):
    input_signal = pre_model.get_layer(name='input_signal').input
    merged = pre_model.get_layer(name='merged').output

    activation_seq = 'relu'

    cnn_part = Model(input_signal, merged) # pre train

    input_seq = Input(shape=(None, 3000, 1)) # sequence 
    # print('input_seq', input_seq.shape)

    signal_sequence = TimeDistributed(cnn_part)(input_seq) # TimeDistributed
...
if __name__=='__main__':
    ...
    seq_model = deepSleepNetwork(pre_model)
